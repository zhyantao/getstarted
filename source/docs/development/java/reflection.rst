...
反射这个词可以形象地理解为镜面反射。也就是说，照镜子时我们可以看见镜子里的自己，那是另一个我，和真的我差别不大。

对于类来讲，也是一样。我们希望在源代码经过了一些处理后，依然能够还原出当时写了什么。也就是说， ``.java`` 
文件经过编译之后，我们可能就不认识字节码文件了，经过反射，我们能够从 ``.class`` 文件还原出 ``.java`` 文件中写了什么，
进而提取出一些有用的信息。
