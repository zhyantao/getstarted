...
JMatPro 是一个用于计算材料性能的软件。它基于预存于软件内部的规则库来计算材料的性能，如导热系数等。
...
因为中重院只购买了通用钢和不锈钢模块，所以我们仅考虑 General Steel 和 Stainless Steel 的相关功能。
...
在配置环境时，若缺少加密狗，则无法继续完成后续工作，参考
[JMatPro API 安装与开发环境配置指南](https://kdocs.cn/l/cd3ZKiq03jt1)。错误提示如下图所示：
...
热力学计算模块，从 General Steel 主界面上可以看出，有下面这几个功能：
