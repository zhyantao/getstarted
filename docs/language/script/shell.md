...
在执行 Shell 脚本时可以向脚本传递参数，脚本内获取参数的格式为 `$n`。其中，`n` 指传递给脚本的第 `n` 个参数。
...
用于检查某个条件是否成立，它可以进行数值、字符和文件三个方面的测试（详见[第3节](https://segmentfault.com/a/1190000037424801#3-运算符)对应的运算符部分）。
