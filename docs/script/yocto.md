...
| `cd $BUILD_DIR && rm -Rf tmp sstate-cache`             | 清除构建缓存                             |
| `bitbake <recipe>`                                     | 构建指定配方                             |
| `bitbake -c clean <recipe>`                            | 清理单个配方的构建产物                        |
| `bitbake -c cleansstate <recipe>`                      | 清理单个配方的共享状态缓存                      |
| `bitbake -e <recipe> \| grep ^S=`                      | 查看源代码提取目录                          |
| `bitbake -e <recipe> \| grep ^WORKDIR=`                | 查看 `WORKDIR` 变量值                   |
| `bitbake-layers show-recipes "gdb*"`                   | 搜索匹配模式的配方                          |
| `bitbake -c devshell <recipe>`                         | 进入命令行交互界面进行编译                      |
| `bitbake -c devpyshell <recipe>`                       | 进入 Python 交互界面进行编译                 |
| `bitbake -c listtasks <recipe>`                        | 列出构建某配方所需的所有任务                     |
| `bitbake -f <recipe>`                                  | 强制重新构建                             |
| `bitbake -v <recipe>`                                  | 详细输出构建过程                           |
| `bitbake -DDD <recipe>`                                | 显示详细 Debug 信息                      |
| `yocto-layer create <layer_name>`                      | 创建新的层                              |
| `bitbake-layers add-layer /path/to/your_meta-layer`    | 添加自定义层到构建系统中                       |
| `bitbake-layers remove-layer /path/to/your_meta-layer` | 从构建系统移除自定义层                        |
