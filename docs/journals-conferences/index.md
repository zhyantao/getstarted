...
```{list-table}
:header-rows: 1
:name: ccf-journals-en
:widths: 7, 10, 33, 7, 7, 33
...
* - 5
  - NLPCC
  - conf.International Conference on Natural Language Processing and Chinese Computing
  - C
  - 会议
  - 人工智能
* - 6
...
for (var i = 0, row; row = tbl1.rows[i]; i++) {
    for (var j = 0, col; col = row.cells[j]; j++) {
        if (col.innerText == "A") {
            row.style.background = "#ffe4c4";
        }
        else if (col.innerText == "B") {
...
var tbl2 = document.getElementById("ccf-conferences-en");
for (var i = 0, row; row = tbl2.rows[i]; i++) {
    for (var j = 0, col; col = row.cells[j]; j++) {
        if (col.innerText == "A") {
            row.style.background = "#ffe4c4";
        }
